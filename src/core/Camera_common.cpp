...
LoadSavedCams(void)
{
	...
	if(f) {
		...
		while(fgets(line, 256, f)) {
			sscanf(line, "%f %f %f  %f %f  %d %d  %d %d", &cs->Source.x, &cs->Source.y, &cs->Source.z, &cs->Alpha, &cs->Beta, &cs->hour,
			       &cs->minute, &cs->oldweather, &cs->newweather);
			cs++;
			numSavedCameras++;
		}
...
SaveCam(CCam *cam)
{
	...
	if(f) {
		fprintf(f, "%f %f %f  %f %f  %d %d  %d %d\n", cam->Source.x, cam->Source.y, cam->Source.z, cam->Alpha, cam->Beta, CClock::ms_nGameClockHours,
		        CClock::ms_nGameClockMinutes, CWeather::OldWeatherType, CWeather::NewWeatherType);
		fclose(f);
		LoadSavedCams();
...
LoadCam(CCam *cam)
{
	if(currentSavedCam < 0 || currentSavedCam >= numSavedCameras) return;
